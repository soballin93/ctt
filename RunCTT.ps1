...
param(
    [string]$ConfigUrl = "https://raw.githubusercontent.com/soballin93/ctt/main/ctt.json"
)
...
try {
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            if (Test-Connection -ComputerName "8.8.8.8" -Count 1 -Quiet) { $hasNet=$true; break }
        } catch { }
    ...
    Write-Log "Launching CTT with preset: $ConfigUrl"
    $psi = New-Object System.Diagnostics.ProcessStartInfo
    $psi.FileName = "$env:WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe"
    $psi.Arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$CTT_File`" -Run -Config `"$ConfigUrl`""
